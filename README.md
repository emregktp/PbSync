...
git clone \[https://github.com/KULLANICI\_ADIN/PbSync.git\](https://github.com/KULLANICI\_ADIN/PbSync.git)
...
\# Uygulama http://SUNUCU\_IP:8000 adresinde çalışacaktır. İlk açılışta sizi Setup ekranına yönlendirecektir.
...
Bilgisayarınıza Rclone indirin ve kurun.

  

Terminali açın ve rclone config yazın.

  

n tuşuna basarak New Remote oluşturun.

  

İsim olarak gdrive verin.

  

Storage türü listesinden Google Drive'ı bulun (genelde 18 numara) ve numarasını yazın.

  

client\_id ve client\_secret kısımlarını boş geçin (Enter).

  

scope olarak 1 (Full access) seçin.

  

root\_folder\_id ve service\_account\_file kısımlarını boş geçin (Enter).

  

Edit advanced config? sorusuna n (Hayır) deyin.

  

Use auto config? sorusuna y (Evet) deyin. Tarayıcınız açılacak, Google hesabınızla giriş yapıp izin verin.

  

İşlem tamamlandığında terminalde y diyerek kaydedin.

  

Son olarak q ile çıkın.
...
token \= {"access\_token":"...","token\_type":"Bearer","refresh\_token":"...","expiry":"..."}
...
Source Selection: Listeden bir VM seçin. Ardından o VM'e ait tarihli bir Snapshot seçin.

  

Target: Yedeğin gönderileceği bulut servisini (gdrive) seçin. İsterseniz Backups/LinuxVMs gibi bir alt klasör belirtebilirsiniz.

  

Browse Files: "Browse Files" butonuna basın. Disk içeriği taranacaktır.
...
Start: "Start Stream Task" butonuna basın.

  

Aşağıdaki siyah pencereden (Log) işlemin durumunu ve yükleme hızını canlı olarak izleyebilirsiniz.
...
Geçici Dosyalar: PbSync, işlem sırasında /mnt/pbsync\_restore klasörünü kullanır. İşlem bittiğinde veya hata aldığında bu klasörü otomatik temizler.
